  ...
  try {
    ...
    if (!req.user?.is_super_admin && req.user?.barbershop_id !== id) {
      return res.status(403).json({
        success: false,
        error: 'Acesso negado. Você só pode acessar sua própria barbearia.'
      });
    }
...
router.put('/:id', authenticateToken, requireRole(['admin']), async (req: AuthRequest, res) => {
  try {
    ...
    if (!req.user?.is_super_admin && req.user?.barbershop_id !== id) {
      return res.status(403).json({
        success: false,
        error: 'Acesso negado. Você só pode atualizar sua própria barbearia.'
      });
    }
...
router.delete('/:id', authenticateToken, requireRole(['admin']), async (req: AuthRequest, res) => {
  try {
    ...
    if (!req.user?.is_super_admin && req.user?.barbershop_id !== id) {
      return res.status(403).json({
        success: false,
        error: 'Acesso negado. Você só pode desativar sua própria barbearia.'
      });
    }
...
router.get('/:id/dashboard', authenticateToken, async (req: AuthRequest, res) => {
  try {
    ...
    if (!req.user?.is_super_admin && req.user?.barbershop_id !== id) {
      return res.status(403).json({
        success: false,
        error: 'Acesso negado. Você só pode acessar o dashboard de sua própria barbearia.'
      });
    }
